    ...
    <div className="flex flex-col min-h-screen bg-slate-50">
      <header className="bg-white px-6 pt-12 pb-6 border-b border-slate-100 sticky top-0 z-40 backdrop-blur-md bg-white/90">
        <div className="flex justify-between items-start mb-6">
          <div>
            <h2 className="text-3xl font-black text-slate-800 tracking-tight leading-none mb-2">{t('hireTalent')}</h2>
            <p className="text-slate-400 text-xs font-bold uppercase tracking-[0.2em]">{t('manageOperations')}</p>
          </div>
          <button 
        ...
        {view === 'workers' && availableWorkers.length > 0 && (
          <div className="mb-2">
            <div className="flex items-center gap-2 mb-4">
               ...
               <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-500">{t('topRated')}</h3>
            </div>
                        ...
                        <div className="flex gap-2 relative z-10" onClick={(e) => e.stopPropagation()}>
                             {isContactUnlocked(worker.uid) ? (
                               <a 
                                  ...
                                  target="_blank"
                                  rel="noreferrer"
                                  className="p-3 bg-slate-900 text-white rounded-2xl shadow-lg hover:bg-pink-600 transition-all active:scale-90 flex items-center justify-center"
                               >
